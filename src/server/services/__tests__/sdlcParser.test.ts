...
describe('sdlcParser', () => {
  describe('status derivation from files', () => {
    it('should derive status "complete" for _COMPLETE.md suffix', async () => {
    ...
    it('should derive status "blocked" for _BLOCKED.md suffix', async () => {
    ...
    it('should derive status "review" when result file exists', async () => {
    ...
    it('should derive status "in-progress" when plan and spec files exist (no result)', async () => {
      const specsDir = mkDir('docs', 'current_work', 'specs');
    ...
    it('should derive status "plan" when only plan file exists', async () => {
      const planDir = mkDir('docs', 'current_work', 'planning');
    ...
    it('should derive status "spec" when only spec file exists', async () => {
      const specsDir = mkDir('docs', 'current_work', 'specs');
    ...
    it('should derive status "idea" for catalog-only entries with no files', async () => {
...
`);
  ...
  describe('empty docs directory', () => {
    it('should return empty array when docs/ does not exist', async () => {
      const results = await parseDeliverables(tmpDir);
      expect(results).toEqual([]);
    });

    it('should return empty array when docs/current_work/ is empty', async () => {
  ...
  describe('deliverable ID extraction', () => {
    it('should extract simple numeric IDs', async () => {
    ...
    it('should extract sub-deliverable IDs with letter suffix', async () => {
      const dir = mkDir('docs', 'current_work', 'specs');
    ...
    it('should extract name from filename (underscores to spaces)', async () => {
      const dir = mkDir('docs', 'current_work', 'specs');
  ...
  describe('getDeliverable', () => {
    it('should find a deliverable by ID (case insensitive)', async () => {
      const dir = mkDir('docs', 'current_work', 'specs');
      touch(dir, 'd1_auth_spec.md');

      const result = await getDeliverable(tmpDir, 'd1');
      expect(result).toBeDefined();
      expect(result!.id).toBe('D1');
    });

    it('should return undefined for non-existent ID', async () => {
      const result = await getDeliverable(tmpDir, 'D999');
      expect(result).toBeUndefined();
    });
  });

  describe('sorting', () => {
    it('should sort deliverables by numeric ID', async () => {
      const dir = mkDir('docs', 'current_work', 'specs');
  ...
  describe('chronicle scanning', () => {
    it('should include files from chronicle directory', async () => {
