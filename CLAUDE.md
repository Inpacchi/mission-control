...
| Idea | `any_name.md` in `current_work/ideas/` | `competitive_audit.md` |

> **Idea auto-discovery:** Any `.md` file in `docs/current_work/ideas/` is auto-discovered by the board as a Deck card. No D-number required — the file gets a synthetic `IDEA-` prefixed ID. Frontmatter (type, complexity, effort, flavor) is optional.
